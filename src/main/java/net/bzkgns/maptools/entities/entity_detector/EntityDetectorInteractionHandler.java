...
public class EntityDetectorInteractionHandler {
    ...
    public static void onLeftClickEntity(AttackEntityEvent event) {
        if (!(event.getTarget() instanceof EntityDetector detector)) return;
        Player player = event.getEntity();
        if (!player.getMainHandItem().is(ModItems.ENTITY_DETECTOR_EDITOR.get())) return;
    ...
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        if (!event.getItemStack().is(ModItems.ENTITY_DETECTOR_EDITOR.get())) return;
